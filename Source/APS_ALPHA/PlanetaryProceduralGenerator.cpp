...
FPlanetarySystemGenerationModel UPlanetarySystemGenerator::GeneratePlanetraySystemModelByStar(FStarGenerationModel StarModel)
{
    ...
    for (double OrbitRadius : OrbitRadii)
    {
        ...
        double MinPlanetMass = 0.1; // минимальная масса планеты в земных массах
        double MaxPlanetMass = 10.0; // максимальная масса планеты в земных массах
        double PlanetMass = FMath::RandRange(MinPlanetMass, MaxPlanetMass); // генерация случайной массы
        PlanetModel.Mass = PlanetMass;

        double PlanetDensity = 5.5; // плотность Земли в г/cm^3
